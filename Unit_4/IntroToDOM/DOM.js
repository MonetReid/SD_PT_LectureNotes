...
- This has various methods available to us.
*/
...
/* 
    Create variable to store a newly made element.
    Our variable now has access to various properties
        - InnerHTML: can inject(we reassign) a block of HTML.
        - innerHTML: returns the visible text contained in a node
...
// Access global document variable, step into with dot notation the document body, use appendChild method to add new element/tag to our HTML body
...
h1.style = `text-shadow: 2px 2px 2px yellow; color: purple; text-Align: center;`
// We can do one off or multiple style attributes 

// JS requires camelCasing when "class" is keyword because its different from CSS to JS that means something different.
...
//? Event Listeners
/* 
    Allow us to execute a function when an even occurs.
    .addEventListener()
        - DOM node method
...
/* 
    ...
    Steps: for adding an Item to the List:
        ...
        - Append the new element to the parent element/ node/parent bucket

    Functionality Step:
        - add event listener to the button
        - use variables to contain the HTML tags we are referencing. (document.getElement.Id)
        - build a function that creates and assigns attributes for the new list elements.
*/
...
//Console.log input node ot know how to grab value
//console.log(inputNode);

// Build our function: use declaration format, name it addItem, no params.
function addItem() {
...
// Build a function to create everything, create/build all elements/ display them.
function buildTable() {
    // testing function with even listener: it worked!
    ...
    //Outside of .forEach but inside the function
