...
const express = require('express'); // being pulled from those node modules.

// We want a simple var to make calls, set express to app

const app = express(); // simplify our method calls

// Create a PORT number to run app/server on our local machine.  
// Server port within local host. Using "all caps" indicates a "general" variable. 
const PORT = 4000;
...
// First we will call on our app var, then we will use the .use() method,
// Create a method that "listens" for us spinning up/starting our application.
// This will need our PORT # and a callback function.

// First we will call on our app var, then we will use the .use() method, we pass use a string that becomes the url route and the controller that route takes you to listen method, pass that method the port var and create a CB function to give us some feedback(console.log) to affirm it's running. 
/* 
