...
router.post("/signup", async (req, res) => {
    ...
    try {
      ...
      // The payload should be the user id, secret message should eventuallly be in the .env
...
router.post("/login", async (req, res) => {
  ...
  try {
//1. Capture data provided by user (body).- obj destructuring
const {email, password} = req.body

        //2. Check database to see if email supplied exists
        // * A MongoDB method that accepts a query as an argument. Returns an instance of a document(user JSON obj) that matches.
        // {email is key for what db is searching for : email = req.body.email from request}
const user = await User.findOne({email: email})
console.log(user);

// Write an error to catch if no one user matches,
