/* 
 ? Classes
    ...
    - We use classes to define a category / set of objects(objects that have similar details but unique )
    - Introduced in 2015
    - A function that helps/allows us to create a specific object(s)
...
/* 
    ...
    2. Class name. Note that class names are done in Pascal Casing to help determine what type of function we are referencing in our code.
    3. Constructor keyword. This is activated when we instantiate (create an instance of) a new object. Builds what sort of keys and values will be associated with our new object. 
    4. The parameters of our constructor object. Established the values of our new object.
    5. This keyword - refers to "this" specific object being created.
    6. The name of a key to our new object. (In this example: an Item object will have 3 keys: name, description, price)
...
3. "this" keyword is referencing "dryGoods" object and adding to it's array of items.
4. Using an array method to push to the items array.
5. Generating/ creating a new department object named dryGoods.
...
class Expense {
    ...
    // only reachable within the class itself not global/ not by children
    ...
    //Expense { purchased_price: 1, sell_at: 1.25 }

    let anotherItemToSell = Expense.addUpchargeForProfit(2)
    console.log(anotherItemToSell);
    //Expense { purchased_price: 2, sell_at: 2.5 }

    //* Class inheritance
    // With each object we create, those methods are included to those specific objects, basically the children inherit from their parent class.
    // anotherItem has the "addTax" method due to inheritance 
    anotherItemToSell.addTax(.06);
    console.log(anotherItemToSell);
